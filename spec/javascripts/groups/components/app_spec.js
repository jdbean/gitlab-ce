...
import { ACTIVE_TAB_OVERVIEW_CHILDREN, ACTIVE_TAB_SHARED } from '~/groups/constants';
...
const createComponent = (hideProjects = false, props = {}) => {
...
describe('AppComponent', () => {
  ...
  describe('created', () => {
    ...
    it('should set `isOverview` prop based on action', done => {
      const newVm = createComponent(false, {
        action: ACTIVE_TAB_OVERVIEW_CHILDREN,
      });
      newVm.$mount();

      Vue.nextTick(() => {
        expect(newVm.isOverview).toBe(true);
        newVm.$destroy();
        done();
      });
    });

    it('should not set `isOverview` prop based on action', done => {
      const newVm = createComponent(false, {
      ...
      Vue.nextTick(() => {
        expect(newVm.isOverview).toBe(false);
        newVm.$destroy();
        done();
      });
    });
